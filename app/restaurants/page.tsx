...
const fallbackRestaurants: RestaurantItem[] = [
  { id: 'seed-16', name: 'Sakhli Traditional Georgian Supra & Wine Cellar', location: 'Tbilisi, Old Town', cuisine: 'Georgian Supra', price: 55, rating: 4.89, img: 'https://images.unsplash.com/photo-1514933651103-005eec06c04b?w=600&h=400&fit=crop' },
  { id: 'rest-2', name: 'Porto Franco Seafood & Wine', location: 'Batumi, Seaside', cuisine: 'Black Sea Seafood', price: 45, rating: 4.75, img: 'https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=600&h=400&fit=crop' },
  { id: 'rest-3', name: 'Café Literaturuli & Wine Bar', location: 'Tbilisi, Vera', cuisine: 'European & Wine', price: 35, rating: 4.65, img: 'https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=600&h=400&fit=crop' },
  { id: 'rest-4', name: 'Wine Garden Qvevri Restaurant', location: 'Sighnaghi, Kakheti', cuisine: 'Farm-to-Table', price: 50, rating: 4.92, img: 'https://images.unsplash.com/photo-1550966871-3ed3cdb51a3a?w=600&h=400&fit=crop' },
];

export default function RestaurantsPage() {
  const [restaurants, setRestaurants] = useState<RestaurantItem[]>(fallbackRestaurants);

  useEffect(() => {
    async function loadRestaurants() {
      try {
        const res = await fetch('/api/listings?category=restaurants&limit=50');
        if (res.ok) {
          const data = await res.json();
          if (data.listings && data.listings.length > 0) {
            const mapped: RestaurantItem[] = data.listings.map((l: any) => ({
              id: l._id || l.id,
              name: l.title,
              location: l.location || l.city,
              cuisine: l.type || 'Georgian Cuisine',
              price: l.price_per_night || l.price || 55,
              rating: l.overall_rating || 4.9,
              img: (l.images && l.images[0]) || fallbackRestaurants[0].img,
            }));
            const existingNames = new Set(mapped.map(m => m.name));
            const remaining = fallbackRestaurants.filter(r => !existingNames.has(r.name));
            setRestaurants([...mapped, ...remaining]);
          }
        }
      } catch (err) {
    ...
    <>
      <main style={{ background: 'var(--bg)', minHeight: '100vh', paddingTop: '80px' }}>
        <section style={{ position: 'relative', padding: '80px 24px 60px', background: 'linear-gradient(135deg, #943d22 0%, #c45632 35%, #9b3d20 70%, #69220e 100%)', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ borderRadius: '20px', background: 'rgba(255, 251, 246, 0.88)', border: '1px solid hsla(0,0%,100%,.35)', backdropFilter: 'blur(24px) saturate(120%)', padding: '48px 40px', textAlign: 'center' }}>
              <h1 style={{ fontFamily: 'var(--font-display), serif', fontSize: 'clamp(2.5rem,6vw,4.5rem)', fontWeight: 400, letterSpacing: '0.06em', margin: '0 0 8px', color: '#1a120e' }}>RESTAURANTS & SUPRAS</h1>
              <p style={{ color: '#5a4538', fontSize: '1.15rem', margin: '0 0 28px' }}>Dine across Georgia — from Tbilisi salon suparas to Kakheti vineyard cellars</p>
        ...
        <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
          <h2 style={{ fontFamily: 'var(--font-display), serif', fontSize: '1.8rem', fontWeight: 400, color: 'var(--ink)', margin: '0 0 32px' }}>Featured Restaurants & Cellars</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '28px' }}>
            {restaurants.map((r) => (
              <Link key={r.id} href={`/listing/${r.id}`} className="card-3d-glow" style={{ textDecoration: 'none', color: 'inherit', display: 'block', borderRadius: '20px', background: 'var(--card-bg, rgba(255, 251, 246, 0.84))', border: '1px solid var(--glass-border, hsla(0,0%,100%,.35))', backdropFilter: 'blur(24px) saturate(120%)', overflow: 'hidden', transition: 'transform 0.3s ease, box-shadow 0.3s ease' }}>
                ...
                <div style={{ padding: '20px 22px 24px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.9rem', color: 'var(--text-secondary, #5a4538)' }}>
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><svg viewBox="0 0 24 24" fill="currentColor" width="14" height="14"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>{Number(r.rating).toFixed(1)}</span>
                  </div>
                </div>
              </Link>
            ))}
